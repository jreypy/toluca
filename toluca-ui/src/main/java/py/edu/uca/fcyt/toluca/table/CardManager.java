...
/**
 * Maneja la animacin de las cartas
 */
class CardManager implements Animable
{
	...
	synchronized private void createTablePlayers()
	{
		...
		Util.verif
		(
			playerCount != -1, "Cantidad de jugadores invlida"
		);
		...
		for (int i = 0; i < playerCount; i++)
		{
			...
			// el orden de dibujo es el orden de creacin
	...
	synchronized public void deal(int dealer, boolean touch)
	{
		...
		// verificaciones
		Util.verifParam(dealer < playerCount, "Parmetro 'dealer' invlido");
	...
	synchronized public TableCard playCard(int player, Card card)
	{
		...
		(
			...
			"Parmetro 'player' invlido: " +  player
		);
     ...
     * @param player	posicin del jugador
     */
...
//			animator.toTop(tDeck.getTableCard(i));
	...
	/**
     * Agrega una pausa a la animacin de las
     * cartas del jugador en la posicin 'pos'
     */
