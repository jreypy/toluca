...
class PlayerManager {
	...
	public synchronized void sitPlayer(TrucoPlayer p, int chair) {
		if (sittedPlayers.contains(p))
			throw new TableException("Jugador ya est sentado: " + p.getName());
...
//		player = (TrucoPlayer) asientos.get(new Integer(chair));
		if (player == null)
			throw new TableException("Silla vaca");
...
//		asientos.remove(new Integer(chair));
	...
	public void setPlayerLocal(TrucoPlayer p) {
		Util.verifParam(p != null, "Parmetro 'p' nulo");
...
/*		if (playerLocal == null)
	...
	/**
	 * Retorna verdadero si los equipos estn parejos
	 */
	 ...
	 * eligi sentarse y de acuerdo a algunos calculos se le coloca en la
	...
	/**
	 * Retorna la posicin en la cual est <code>player</code>. Si no
	 * est sentado, retorna -1
	 */
