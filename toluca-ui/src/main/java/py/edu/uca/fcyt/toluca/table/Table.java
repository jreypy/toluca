...
public class Table implements PTableListener, ChatPanelContainer,
    ...
    /** El player que pertenece al login actual, o sea el "dueo" de este RoomClient */
    private TrucoPlayer playerLocal; 

    private int tableNumber; // nmero de mesa

    private FaceManager fManager; // FaceManager asociado

    private Vector signs; // vector de seas
    ...
    public void initialize() {
        ...
        // remueve manejador de cartas de la animacin
        ...
        if (status == SIT)
            ...
            if (status == PLAYCANTO || status == PLAY) {
                if (e.getButton() == MouseEvent.BUTTON1) {
                    ...
                    if (status == PLAYCANTO)
                        // si hay que responder algn canto,
                        // no jugar la carta
                        card = null;
                    else
            ...
            // abrir las cartas de algn jugador si
    ...
    protected boolean sitIfClick(int x, int y) {
        ...
        // carga la posicin clickeada
        pos = fManager.getPosIfClick(x, y);

        // si existe dicha posicin...
        if (pos != -1) {
            TrucoPlayer player;
            int chair, localChair;

            // obtiene la silla en esa posicin
    ...
    /**
     * Muestra las seas a un cierto jugador si se clicke en l
     */
    protected boolean sendSignsIfClick(int x, int y) {
        int pos;
        TrucoPlayer player;

        // obtiene la posicin de jugador al
        // cual se debe enviar las seas
        pos = fManager.getPosIfClick(x, y);
        ...
        // si es la posicin del contrincante, salir
        if (pos % 2 == 0)
            return false;

        // obtiene el jugador sentado en esa posicin
        player = pManager.getPlayer(pManager.getChair(pos + 1));

        // si no hay seas, informa a todos los
        // TableListeners registrados la sea "seca"
        if (signs.size() == 0)
            tEventMan.fireSignSendRequest(player, Sign.SECA);
        // si las hay informa a todos los
        // TableListeners registrados las seas
        else
    ...
    /**
     * Muestra la sea emitida por un cierto jugador
     */
    public void showSign(TableEvent event) {
        TrucoPlayer src, dest;
        StringBuffer log = new StringBuffer("seas -> ");
        ...
        // avisa que la sea fue enviada correctamente
        tEventMan.fireSignSent(event.getPlayer(1));
    }

    /**
     * Agrega una sea a emitir posteriormente
     */
    protected void addSign(int sign) {
        // verificaciones
        Util.verifParam(Sign.isSign(sign), "Sea invlida");
    ...
    public void showChatMessage(TrucoPlayer player, String htmlMessage) {
        ...
        // si el mensaje comienza con una barra, va slo para
        // el chat
