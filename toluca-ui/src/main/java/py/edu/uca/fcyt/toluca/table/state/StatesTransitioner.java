...
{
	// Contiene el par (estado, duracin)
	...
	private long lastTime = -1;			// ltimo tiempo de avance
     ...
     * tiene una cola de estados vaca.
     */								
     ...
     * del mismo pasndole como parmetro el siguiente estado y
     * la cantidad de cambio (0 a 1). A la vez, consume el tiempo 
     * del estado actual en una cantidad igual al tiempo 
     * transcurrido entre la ltima vez que se llam a la funcin
     * y el momento en el cual se la llama. Si se la llama por
     * primera vez, el ltimo tiempo es considerado es tiempo
     * en el cual se agreg el primer elemento de la cola.
     * @return 	Verdadero si pudo avanzar. Falso si no hay 
     * ms estados en la cola
     */
	synchronized public boolean advance()
	{
		...
		// si no hay ltimo tiempo de avance, que sea el actual
		if (lastTime == -1) 
		{
			lastTime = actualTime;
			return false;
		...
		// obtiene el tiempo transcurrido desde el ltimo avance
		...
		// el estado actual a 'elapsedTime' milisegundos despus
     ...
     * el actual es el agregado, y su duracin se omite.
     * @param state		Estado a agregar. Si es nulo, se agrega una pausa.
     * @param duration	Duracin del estado.
     */
	synchronized public void pushState(State state, long duration)
	{
		...
		// verificaciones
//		Util.verifParam(state != null, "Parmetro 'state' nulo");
		Util.verifParam
		(
			duration >= 0, "Parmetro duration invlido: " + duration
		);
		
		// actualiza el tiempo restante para quedar inactiva
		remainingTime += duration;

		// si el estado actual es nulo, que sea ste
...
//		{
			...
			// si no haba siguiente estado, el agregado es el
			// siguiente y el tiempo de ltimo avance es el actual
			if (nextState == null)
			{
...
//		}
    ...
    {
    	// si la duracin es 0, no se hace nada
...
//    		}
    ...
    /**
     * Retorna el ltimo estado de la cola
     */
