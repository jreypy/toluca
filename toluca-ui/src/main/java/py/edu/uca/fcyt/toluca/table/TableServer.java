...
//        logger.log(Level.WARNING, player.getName() + " sitted in server chair " + chair + " in table of " + getHost().getName());
    ...
    public void startGame(int points) {
        ...
        tGame = new TrucoGameServer(tTeams[0], tTeams[1], points);
    ...
    public void endOfGame(TrucoEvent event) {
        ...
        TrucoGameServer tg = (TrucoGameServer) event.getGame();
...
//            logger.log(Level.WARNING, "tableListeners.size() = "+tableListeners.size());
    ...
    public TrucoGameServer getTrucoGame() {
        return tGame;
    }
    ...
    public void kickPlayer(TrucoPlayer playerKicked) {
        ...
        if (chair >= 0) {//si estaba sentado primero que se pare
            ...
            TrucoGameServer trucoGame = getTrucoGame();
            if (trucoGame != null) {//que vea si hay juego

                if (trucoGame.getTeam(0).getPlayer(playerKicked.getName()) != null) {//si el team 0 le tenia tiene que ganar el team 1


                    trucoGame.setTeamGanador(1);

                }
                if (trucoGame.getTeam(1).getPlayer(playerKicked.getName()) != null) {//si el team 1 le tenia tiene que ganar el team 0
...
//		TableEvent te = new TableEvent(TableEvent.EVENT_playerKicked, this, tptmp, null,0);
    ...
    public void comprobarTable() {
        if (getPlayers().size() == 0) {//hay que eliminar la tabla
    ...
    class TrucoGameServer extends TrucoGameImpl {
        private int teamGanador;

        public TrucoGameServer(TrucoTeam tTeam, TrucoTeam tTeam1, int points) {
            super(tTeam, tTeam1, points);
        }
        ...
        @Override
        public void setTeamGanador(int teamGanador) {
            this.teamGanador = teamGanador;
        }

        public List getListaListeners() {
