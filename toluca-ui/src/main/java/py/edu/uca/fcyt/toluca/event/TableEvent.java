...
public class TableEvent {
	...
	) 
	{
		...
		Util.verifParam(table != null, "Parmetro 'table' nulo");
	...
	/**
	 * Retorna una identificacin que contiene informacin sobre
	 * los parmetros activos de este TableEvent y el evento
	 * que se gener.
	 */
	public int getID() {
		int p, v;
	...
	public TableServer getTableServer() {
		...
		/**
		 * Ejemplo de uso. La mscara juega el rol de tipo de
		 * evento y las 'isXSpecified' el rol de determinadores
		 * de existencia o inexistencia de un cierto parmetro
		 * para un cierto tipo (mscara).
		 * El parmetro Table siempre est especificado.
		 */
  /*  public static void main(String[] args) {
		...
		// combinaciones de parmetros activos
		...
		// parmetros de cada TableEvent
		for (int i = 0; i < 4; i++) {
			// obtiene la msca del 'i'-simo TableEvent
			id = tEvents[i].getID();
   
			// imprime la existencia o inexistencia
			// de c/u de los parmetros
