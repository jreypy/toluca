...
class TTextAnimator implements Animable, StateListener
{
	...
	/** Muestra la presentacin inicial */
	...
	private void createTitle()
	{
		...
		// posicin inicial horizontal y ngulo inicial
		pos = -(int) (size * 1.175);
		ang = -Math.PI * .4;
		...
		{
			// el texto comenzar con una posicin elptica
			rad = 1000;
			x = (int) (rad * Math.sin(ang));
			y = (int) (rad * Math.cos(ang)) + 100;
			y *= ((i % 2) * 2 - 1) * .3;

			// crea el TableText letra y lo agrega al ttulo
	...
	private void createSubTitle()
	{
		...
		// la animacin anterior
		remTime = getRemainingTime();

		// el subttulo estar a 100 puntos por debajo del centro
		...
		tTexts = new TableText[]
		{
			...
			new TableText("Universidad Catlica", font, 2, Math.PI * 1.5),
			new TableText("de Asuncin", font, 2, Math.PI * 1.5)
		};
		
		// agrega los textos al subttulo
		for (int i = 0; i < tTexts.length; i++)
		{
			// agrega el texto y carga sus datos de animacin
		...
		// carga el subttulo al vector
		ttGroups.insertElementAt(ttgSubTitle, 0);
	}
	
	/** Desvanece la presentacin */
...
//		Thread th;
	...
	/** Normaliza la duracin de todos los TTextGroups.
	 * Agrega una pausa a todos los TableTexts de tal
	 * manera que todas terminen al mismo tiempo. El
	 * TTextGroup de mayor duracin no tiene pausa agregada.
	 */
	...
	/** Elimina cualquier animacin actual */
