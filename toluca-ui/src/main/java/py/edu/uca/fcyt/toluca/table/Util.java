...
public class Util
{
	...
	{
		// verificaciones
		Util.verifParam(iIcon != null, "Par?metro 'iIcon' nulo");
		Util.verifParam(bImage != null, "Par?metro 'bImage' nulo");
	...
	/**
	 * si 'exp' es falso arroja una excepci?n 
    ...
    public static String getKindName(int kind)
    {
    	switch (kind)
    	{
    		...
    		default:
    			...
    			(
    				"Argumento 'kind' inv?lido"
    			);
    	}
    }

	/** 
	 * Convierte 'ang' en un ?ngulo en el rango [-PI/2, PI/2)
	 */
    ...
    /**
     * Retorna el subdirectorio de im?genes
     */
	 ...
	 * en varias l?neas de manera que su ancho no sea mayor
	 * que <code>width</code>.
	 * @param text	Texto a analizar
	 * @param width	Tama?o m?ximo en puntos
	...
	)
	{
		...
		// crea la lista enlazada de l?neas
		lines = new LinkedList();
		
		// obtiene los t?kens (palabras) de 'text'
		strTok = new StringTokenizer(text, " ", true);
		
		// obtiene las medidas de la fuente de 'grOut'
		fMetrics = grOut.getFontMetrics();
		
		// inicializa el tama?o actual de la l?nea como para 
		// que al principio sobrepase el tama?o m?ximo
		lineSize = 0;
		
		// inicaliza la l?nea actual
		currLine = new String();
		
		// carga las l?neas de texto
		while(strTok.hasMoreElements())
		{
			// obtiene el t?ken y su tama?o
			token = (String) strTok.nextElement();
			tokWidth = fMetrics.stringWidth(token);
			
			// si el tama?o del t?ken es mayor que 'width'
			...
			// incrementa el tama?o actual de la l?nea
			lineSize += tokWidth;
			
			// si ha excedido el tama?o, habilitar una nueva l?nea
			if (lineSize > width)
			{
				lineSize = tokWidth;
				lines.add(currLine);
			...
			// si no, agregar el t?ken a la l?nea
	 ...
	 * en una cierta cantidad de p?xeles a lo largo del texto.
	 * @param text	Texto a analizar.
	 * @param start	Posici?n inicial del an?lisis.
	 * @param width	Tama?o m?ximo en p?xeles.
	...
	)
	{
		...
		// carga el tama?o actual y la cantidad 
