...
public class StatusPlayer {
    protected Logger logeador = Logger.getLogger(StatusPlayer.class.getName());
    ...
    private int valor_envido(TrucoCard cual) {// Devuelve el valor de la carta en el envido
        if (cual.getValue() >= 10)
            return 0;
        else
    ...
    private boolean son_mismo_palo(TrucoCard uno, TrucoCard dos) { //Verdadero si son del mismo palo
    ...
    public boolean jugarCarta(TrucoCard cual) {
        ...
        for (int i = 0; i < 3; i++) {
            if (cartas[i] == null) {
                logeador.warning("El PLayer no tiene Carta en la posición [" + i + "]");
            } else if (cartas[i].equals(cual) && cartas[i].isFlipped() == false) {
        ...
        logeador.warning("No se encontró la Carta para jugar [" + cual + "]");
        return false;
    }
     ...
     */
    public int puedeJugarCarta(TrucoCard cual) {//Busca la carta y verifica si la carta no se jugo ya
        if (cual == null) {
        ...
        if (!seCerro) {
            for (int i = 0; i < 3; i++) {
                ...
                if (cartas[i].equals(cual) && cartas[i].isFlipped() == false) {
                    System.out.println("status player 1 - ok - OK ");
                    return 1;
                } else if (cartas[i].equals(cual) && cartas[i].isFlipped() == true) {
                    System.out.println("Cartas de Jugador [" + cartas + "]");
                    System.out.println("status player 2 - ok - Quiere Volver a Jugar una Carta Jugada");
                    return 0;
                }
            }
        }
        System.out.println("status player 3 - ok - Está Cerrado");
        return -1;
    }
