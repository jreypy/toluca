...
import py.edu.uca.fcyt.toluca.game.TrucoHand;
...
public class TrucoStatusTable {
    ...
    protected StatusPlayer[] estado;
    protected StatusEnvido envidos;
    protected TrucoDeck elMazo;
    protected StatusFlor flores;
    protected StatusMano mano;


    private void repartir() //reparte  3 cartas a todos los jugadores
    {
        logger.debug("Giving Cards ");
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < cJugadores; j++) {
                TrucoCard card = elMazo.getTopCard();
    ...
    public TrucoStatusTable(int cantidadDeJugadores) {
        ...
        logger.debug("Creating TrucoStatusTable [" + elMazo + "]");
        repartir();
    }
...
//--------------------------------------------------------------------------------------------------------
    ...
    public boolean jugarCarta(int jugador, TrucoCard carta) {
        logger.debug("Jugador [" + jugador + "] plays card [" + carta + "]");
